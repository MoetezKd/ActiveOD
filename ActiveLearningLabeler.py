...
__appname__ = 'ActiveLearning Labeler'
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        retrain = action(get_str('retrainModel'), self.retrain_model,
                        'space', 'retrain', get_str('retrainModelDetail'))
        ...
        self.actions.beginner = (
            open_dir,  open_next_image, open_prev_image, retrain, save, save_format_beginner, autolabel, create, copy, delete,
    ...
    def auto_label(self):
        attention_get = QMessageBox.question(self, 'Attention', 'This operation will clear all the original annotation files. Click Yes to select the yolov5 weight and continue.', QMessageBox.Yes | QMessageBox.No)  
        if attention_get == QMessageBox.Yes:
...
#saved_path
    ...
    def retrain_model(self, _value=False):
        attention_get = QMessageBox.question(self, 'Attention', 'This action will retrain the model it will take some time to finish it. See terminal', QMessageBox.Yes | QMessageBox.No)  
        if attention_get == QMessageBox.Yes:
            #moving all test unlabled images and annotations to the training folder (to complete)
            if sys.platform=="linux" or sys.platform=="linux2":
                os.system("cp -r ./images/*.png ./yolov5/data/images/train/")
                os.system("cp -r ./images/*.txt ./yolov5/data/labels/train/")
            elif sys.platform=="win32":
                os.system("robocopy images %cd%/yolov5/data/images/train/")
            #starting training (to complete)
            os.system("python ./yolov5/train.py --data ./yolov5/data/mobility_aid.yaml --weights  ./yolov5/runs/train/$(ls ./yolov5/runs/train | tail -1)/weights/best.pt")
            return
